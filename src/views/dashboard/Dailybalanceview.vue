  ...
  <div class="space-y-6">
    ...
    <template v-else>
      ...
      <div class="card p-5">
        ...
        <!-- Where the income actually came from: real Deposits/Withdrawals -->
        <div v-if="today?.income_transactions?.length" class="mt-4">
          <button @click="showIncomeTransactions = !showIncomeTransactions" class="text-xs font-medium" style="color:#938af4">
            {{ showIncomeTransactions ? 'Hide' : 'View' }} transactions behind this income ({{ today.income_transactions.length }})
          </button>
          <div v-if="showIncomeTransactions" class="mt-2 border border-gray-100 rounded-lg overflow-hidden">
            <table class="w-full text-sm">
              <thead class="bg-gray-50 border-b border-gray-100">
                <tr>
              ...
              <tbody class="divide-y divide-gray-50">
                <tr v-for="(t, idx) in today.income_transactions" :key="idx">
                  <td class="table-cell">
                    <span :class="['badge', t.type === 'withdrawal' ? 'bg-orange-100 text-orange-700' : 'bg-emerald-100 text-emerald-700']">
                      {{ t.type === 'withdrawal' ? 'Withdrawal' : 'Deposit' }}
                    </span>
                  </td>
                  <td class="table-cell font-mono text-xs">{{ t.transaction_no || '—' }}</td>
                  <td class="table-cell text-gray-700">{{ t.client_name || '—' }}</td>
                  <td class="table-cell text-sm text-gray-500 whitespace-nowrap">{{ fmtTime(t.date) }}</td>
                  <td class="table-cell font-mono text-xs text-gray-500">{{ t.currency || 'USD' }}</td>
                  <td class="table-cell text-right font-mono" :class="t.type === 'withdrawal' ? 'text-orange-600' : 'text-emerald-600'">
                    {{ t.type === 'withdrawal' ? '-' : '+' }}{{ fmtAmt(t.amount, t.currency) }}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <tbody class="divide-y divide-gray-50">
              ...
              <template v-else v-for="row in history" :key="row.id">
                <tr class="hover:bg-gray-50/60">
                  <td class="table-cell">
                    <button v-if="row.details?.length" @click="toggleRow(row.id)" class="btn-icon">
                      <ChevronRightIcon class="w-4 h-4 transition-transform" :class="{ 'rotate-90': expandedRows.has(row.id) }" />
                    </button>
                  </td>
                  <td class="table-cell font-medium text-gray-800">{{ fmtDate(row.date) }}</td>
                  ...
                  <td class="table-cell text-sm text-gray-600">{{ row.closed_by?.name || '—' }}</td>
                </tr>
                <tr v-if="expandedRows.has(row.id) && row.details?.length" class="bg-gray-50/60">
                  <td></td>
                  <td colspan="15" class="px-4 pb-3">
                    <p class="text-xs font-semibold text-gray-500 mt-2 mb-1">Opening detail</p>
                    <table class="w-full text-xs border border-gray-200 rounded overflow-hidden bg-white mb-3">
                      <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                          <th class="table-header">Type</th>
                      ...
                      <tbody class="divide-y divide-gray-50">
                        <tr v-for="d in row.details.filter(d => d.phase === 'open')" :key="`open-${d.id}`">
                          <td class="table-cell">{{ d.entity_type === 'product_type' ? 'Product' : 'Company Bank' }}</td>
                          <td class="table-cell">{{ d.label || '—' }}</td>
                          <td class="table-cell font-mono">{{ d.currency || 'USD' }}</td>
                          <td class="table-cell text-right font-mono">{{ d.currency === 'KHR' ? khr(d.amount) : usd(d.amount) }}</td>
                        </tr>
                      </tbody>
                    </table>
                    <template v-if="row.details.some(d => d.phase === 'close')">
                      <p class="text-xs font-semibold text-gray-500 mb-1">Closing detail</p>
                      <table class="w-full text-xs border border-gray-200 rounded overflow-hidden bg-white">
                        <thead class="bg-gray-50 border-b border-gray-100">
                          <tr>
                            <th class="table-header">Type</th>
                            <th class="table-header">Label</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                          <tr v-for="d in row.details.filter(d => d.phase === 'close')" :key="`close-${d.id}`">
                            <td class="table-cell">{{ d.entity_type === 'product_type' ? 'Product' : 'Company Bank' }}</td>
                            <td class="table-cell">{{ d.label || '—' }}</td>
                            <td class="table-cell font-mono">{{ d.currency || 'USD' }}</td>
                            <td class="table-cell text-right font-mono">{{ d.currency === 'KHR' ? khr(d.amount) : usd(d.amount) }}</td>
...
import { startTodayBalance, closeTodayBalance, getTodayBalance, getBalanceHistory } from '@/api/daily-balances'
...
const showIncomeTransactions = ref(false)
